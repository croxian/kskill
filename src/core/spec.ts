 ...
 *
 * 30초 → 15초 → 10초 → 5초 → 3초로 내려왔다. 이 값들 중 무엇도 측정으로
 * 정당화된 적이 없다. 10초에서 30분을 돌려 밀려나지 않았다는 것이 우리가
 * 가진 전부다.
 *
 * 3초로 온 이유: 발견이 몇 초 빠른 게 실제로 값이 있다고 사용자가 판단했다.
 * 화면을 보고 있으면 알림에서 결제까지의 지연이 짧아서, 앞단 3초가 뒷단에
 * 묻히지 않는다. 그 판단은 쓰는 사람 몫이다.
 *
 * 시간당 요청보다 **한 번의 감시가 통째로 만드는 요청 수**가 더 중요하다.
 * 상영 20시간 전부터 지켜보면 10초는 7천 회, 3초는 2만 4천 회다.
 * 시간당 수치만 보면 그 차이가 안 보여서 콘솔에 총량을 같이 띄운다.
 *
 * 여기서 더 내릴 생각이라면 간격이 아니라 **감시 시간**을 줄이는 쪽을
 * 먼저 보는 게 낫다. 20시간을 3초로 보는 것보다 마지막 3시간을 3초로 보는
 * 편이 총량이 7분의 1이고, 취소표는 그 구간에 몰린다.
 */
export const POLL_FLOOR_SEC = 3;
