...
> **먼저 읽으세요 — 서버에서는 막혔습니다 (2026-09-23).**
>
> Oracle Cloud VM 에서 하루는 정상 동작했으나(2026-09-22), 다음 날 로그인이
> 거부되기 시작했습니다. 코레일이 403 과 함께 이렇게 답합니다.
>
> ```
> {"code":-2000,"message":"원활한 서비스를 위해 안정적인 환경에서 앱을
>  이용해 주시기 바랍니다. 매크로 등 미허가 도구 사용 시 이용이 제한될 수
>  있습니다. ..."}
> ```
>
> 같은 시각 같은 계정이 **집 PC 에서는 정상 동작**했고, 코레일톡 앱 로그인도
> 멀쩡했습니다. 계정 문제도 네트워크 차단도 아니고, 코레일의 매크로 탐지가
> 서버에서 온 로그인을 거부한 것입니다. 첫 요청(암호화 키)은 200 으로
> 통과하고 로그인 요청만 403 입니다.
>
> 우회 방법은 이 저장소에서 다루지 않습니다. 코레일이 명시적으로 거부한
> 것을 뚫는 일이고, 계속 시도하면 계정이 제한될 수 있습니다.
>
> 아래 클라우드 배포 문서는 **기록으로 남겨둡니다.** systemd 배포나 텔레그램
> 봇 구조는 다른 프로젝트에 그대로 쓸 수 있습니다. 다만 코레일 감시 용도로는
> 지금 동작하지 않습니다.
...
덮어쓸 때는 원래 내용을 백업해 두고 알려줍니다. 잃어버리지는 않습니다.

```
직접 고쳐져 있던 파일을 덮어썼습니다. 원래 내용은 백업해 뒀습니다:
  /opt/ktx-bot/ktx_watch.py.bak.20260922-143459
되살리려면:  sudo cp <백업파일> /opt/ktx-bot/<원래이름>
```

백업 목록 보기:

```bash
ls -l /opt/ktx-bot/*.bak.*
```
...
`check_waitlist.py` 가 `[예약대기]` 를 표시할 수는 있지만, **실제로 쓸 수
있는 경우는 드뭅니다.** 정작 필요한 매진 시간대에는 예약대기가 열리지 않는
경우가 많다는 것이 실제 사용에서 확인됐습니다(2026-09).

따라서 예약대기를 전제로 계획하지 마시고, 취소표 감시나 다른 교통편을
기준으로 잡으세요.
...
- 예약대기는 매진 시간대에 잘 열리지 않습니다. 대안으로 삼기 어렵습니다.
